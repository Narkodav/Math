...
public:
	...
	BigFloat operator+(const BigFloat& other) const
	{
		...
		if (m_exponent == other.m_exponent) return BigFloat(std::move(m_mantissa + other.m_mantissa), m_exponent);

		if (m_exponent > other.m_exponent)
		{
			BigInt shiftedMantissa = m_mantissa << (m_exponent - other.m_exponent);
			return BigFloat(std::move(shiftedMantissa + other.m_mantissa), other.m_exponent);
		}
		else
		{
			BigInt shiftedMantissa = other.m_mantissa << (other.m_exponent - m_exponent);
			return BigFloat(std::move(shiftedMantissa + m_mantissa), m_exponent);
		}
	}
