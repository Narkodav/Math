...
namespace Math::BigNum {
	...
	public:
		...
		static inline const bool s_positive = false;
		static inline const bool s_negative = true;

	private:
		BigUint m_num;
		bool m_sign = s_positive;	

	public:

		BigInt() : m_num(), m_sign(s_positive) {};
		...
		BigInt(const BigUint& other, bool sign = s_positive) noexcept;
		BigInt(BigUint&& other, bool sign = s_positive) noexcept;
		...
		BigInt operator*(const BigInt& other) const noexcept
		{
			return BigInt(m_num * other.m_num, m_sign == other.m_sign ? s_positive : s_negative);
		}
		...
		{
			if (m_sign == s_negative && other.m_sign == s_positive)
				return true;
			else if (m_sign != other.m_sign)
				return false;
			if (m_sign == s_positive)
		...
		bool operator>(const BigInt& other) const noexcept
		{
			if (m_sign == s_positive && other.m_sign == s_negative)
				return true;
			else if (m_sign != other.m_sign)
				return false;
			if (m_sign == s_positive)
